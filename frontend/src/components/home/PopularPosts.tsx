...
export default function PopularPosts({ posts }: PopularPostsProps) {
  return (
    <section className="space-y-6">
      ...
      {posts.length === 0 ? (
        <Card className="border-dashed border-amber-200/70 dark:border-amber-900/50 bg-amber-50/40 dark:bg-amber-950/10">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-amber-100 dark:bg-amber-950/40">
              <svg
                className="h-7 w-7 text-amber-500 dark:text-amber-300"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-200">
              아직 인기글을 집계할 만큼 조회 데이터가 쌓이지 않았습니다.
            </p>
            <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
              글을 읽기 시작하면 이 영역에 조회수가 높은 글이 표시됩니다.
            </p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <Card className="h-full group hover:shadow-2xl hover:border-amber-200 dark:hover:border-amber-800/50 transition-all duration-500 cursor-pointer overflow-hidden border-zinc-100 dark:border-zinc-800 dark:bg-[#0a0a0a] flex flex-col">
                    {imageSrc ? (
                      ...
                      <div className="aspect-video bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center group-hover:bg-amber-50 dark:group-hover:bg-amber-950/20 transition-colors">
                        <svg
                          className="w-12 h-12 text-zinc-300 dark:text-zinc-700 group-hover:text-amber-200 dark:group-hover:text-amber-800/50 transition-colors"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
